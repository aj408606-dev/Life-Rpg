...
const GlobalCSS = () => (
  <style>{`
    ...
    .dark .glass-panel { background: rgba(15, 23, 42, 0.75); border: 1px solid rgba(255, 255, 255, 0.08); box-shadow: 0 10px 40px rgba(0, 0, 0, 0.3); }
    ...
    .face { position: absolute; display: flex; align-items: center; justify-content: center; background-color: var(--c); font-size: 1.8rem; border: 1px solid rgba(0,0,0,0.2); box-shadow: inset 0 0 12px rgba(0,0,0,0.15); }
    
    .front { width: var(--w); height: var(--h); transform: translateZ(calc(var(--d) / 2)); }
    .right { width: var(--d); height: var(--h); transform: rotateY(90deg) translateZ(calc(var(--w) / 2)); filter: brightness(0.75); }
    .top   { width: var(--w); height: var(--d); transform: rotateX(90deg) translateZ(calc(var(--h) / 2)); filter: brightness(1.15); }
    
    /* Face Graphics mapping */
    .c-head  { --w: 44px; --h: 44px; --d: 44px; --c: #f0c396; top: 20px; left: 58px; z-index: 10; }
    ...
    .c-body  { --w: 44px; --h: 66px; --d: 22px; --c: #3b82f6; top: 64px; left: 58px; z-index: 5; }
    .c-arm-l { --w: 18px; --h: 60px; --d: 22px; --c: #f0c396; top: 64px; left: 40px; transform: translateZ(-2px); z-index: 4; }
    .c-arm-r { --w: 18px; --h: 60px; --d: 22px; --c: #f0c396; top: 64px; left: 102px; transform: translateZ(2px); z-index: 6; }
    .c-leg-l { --w: 21px; --h: 60px; --d: 22px; --c: #1e3a8a; top: 130px; left: 58px; z-index: 4; }
    .c-leg-r { --w: 21px; --h: 60px; --d: 22px; --c: #1e3a8a; top: 130px; left: 81px; z-index: 5; }
    ...
    .c-pet .face { background: transparent; border: none; font-size: 2.2rem; filter: drop-shadow(0 10px 10px rgba(0,0,0,0.3)); box-shadow: none; }
...
export default function LifeRPGApp() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const savedTheme = localStorage.getItem("liferpg.theme");
    ...
    <>
      ...
      {!sessionUser ? (
        ...
        <div className="text-slate-900 dark:text-slate-100 transition-colors duration-300 pb-24 md:pb-8">
          <Header user={sessionUser} theme={theme} toggleTheme={() => setTheme(t => t === 'dark' ? 'light' : 'dark')} onLogout={handleLogout} nav={nav} setNav={setNav} />
          ...
          <main className="max-w-7xl mx-auto p-4 md:p-6 mt-4">
            ...
            {nav === 'leaderboard' && <Leaderboard localDB={localDB} />}
          </main>
...
function AuthScreen() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (mode === 'signup') {
      if (!name) {
        setLoading(false);
        return showError("Choose an adventurer name.");
      }
      ...
      if (authError) {
        setLoading(false);
        return showError(authError.message);
      }

      if (authData.user && !authData.session) {
        setLoading(false);
        setInfo("Confirmation link sent! Check your email to verify your account.");
        return;
      }
    } else {
      ...
      if (signInError) {
        setLoading(false);
        return showError(signInError.message);
      }
    }
  ...
  const handleOAuth = async (providerName: string) => {
    const provider = providerName.toLowerCase() as 'google' | 'apple' | 'facebook';
    ...
    if (error) showError(`Authentication failed: ${error.message}`);
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-md glass-panel rounded-[2rem] p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="text-center mb-8 mt-2">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-3 font-medium">Authenticate to enter the realm.</p>
        </div>
        ...
        <div className="flex gap-2 mb-6 bg-slate-100/50 dark:bg-slate-800/50 p-1.5 rounded-xl border border-slate-200 dark:border-slate-700">
          <button type="button" onClick={() => { setMode('login'); setError(''); setInfo(''); }} className={`flex-1 py-2.5 rounded-lg font-bold transition-all ${mode === 'login' ? 'bg-white dark:bg-slate-700 shadow-sm text-[#8b5cf6] dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>Login</button>
          <button type="button" onClick={() => { setMode('signup'); setError(''); setInfo(''); }} className={`flex-1 py-2.5 rounded-lg font-bold transition-all ${mode === 'signup' ? 'bg-white dark:bg-slate-700 shadow-sm text-[#8b5cf6] dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>Sign up</button>
        </div>
        ...
        <form onSubmit={handleSubmit} className={`space-y-4 ${shake ? 'shake' : ''}`}>
          ...
          {info && <p className="text-[#10b981] text-sm font-bold text-center">{info}</p>}
          
          <button type="submit" disabled={loading} className="w-full py-4 mt-2 rounded-xl bg-gradient-to-r from-[#8b5cf6] to-purple-600 text-white font-bold text-lg shadow-[0_10px_20px_rgba(139,92,246,0.3)] hover:shadow-[0_10px_25px_rgba(139,92,246,0.5)] transition-all card-hover disabled:opacity-50">
            {loading ? "Casting Spell..." : mode === 'signup' ? "Create Hero Account" : "Enter the Realm"}
          </button>
        </form>
        ...
        <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-700/50">
          ...
          <div className="grid grid-cols-3 gap-3">
            <button type="button" onClick={() => handleOAuth('google')} className="flex justify-center py-3 rounded-xl bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-700 hover:scale-105 transition-transform shadow-sm">
              <svg className="w-6 h-6" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.58c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
            </button>
...
function Header({ user, theme, toggleTheme, onLogout, nav, setNav }: any) {
  return (
    <header className="sticky top-0 z-30 glass-panel border-b-0 shadow-sm">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center">
        ...
        <nav className="hidden md:flex gap-8 ml-12 items-center">
          {[{ id: 'dashboard', label: 'Dashboard' }, { id: 'shop', label: 'Equipment Shop' }, { id: 'leaderboard', label: 'Hall of Heroes' }].map(item => (
            <button key={item.id} onClick={() => setNav(item.id)} className={`text-base font-bold pb-1 transition-all border-b-2 ${nav === item.id ? 'text-[#8b5cf6] dark:text-white border-[#8b5cf6]' : 'text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 border-transparent'}`}>
              {item.label}
            </button>
          ))}
        </nav>
        ...
        <div className="ml-auto flex items-center gap-4">
          <span className="glow-gold px-4 py-2 rounded-full font-bold bg-white dark:bg-[#0f172a] text-[#e8c547] text-sm shadow-md">{user.gold} G</span>
          <button onClick={toggleTheme} className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center hover:scale-110 transition-transform text-xl">{theme === 'dark' ? '☀️' : '🌙'}</button>
          <button onClick={onLogout} className="w-10 h-10 rounded-full border border-slate-300 dark:border-slate-600 flex items-center justify-center hover:bg-[#f43f5e] hover:text-white transition-colors">🚪</button>
        </div>
      </div>
    </header>
  );
...
function MobileNav({ nav, setNav }: { nav: string, setNav: (s: any) => void }) {
  const items = [{ id: 'dashboard', icon: '🏠', label: 'Home' }, { id: 'shop', icon: '🛍️', label: 'Shop' }, { id: 'leaderboard', icon: '🏆', label: 'Ranks' }];
  return (
    <nav className="md:hidden fixed bottom-0 w-full glass-panel border-t border-slate-200 dark:border-slate-800 flex justify-around p-4 z-40 pb-safe shadow-[0_-10px_30px_rgba(0,0,0,0.1)]">
      {items.map(item => (
        <button key={item.id} onClick={() => setNav(item.id)} className={`flex flex-col items-center gap-1 transition-all ${nav === item.id ? 'text-[#8b5cf6] dark:text-white transform scale-110' : 'text-slate-400 hover:text-slate-600'}`}>
          <span className="text-2xl">{item.icon}</span><span className="text-[10px] font-bold uppercase">{item.label}</span>
        </button>
      ))}
    </nav>
  );
        ...
        <div className="glass-panel rounded-[2rem] p-8 text-center relative overflow-hidden">
          ...
          <div className="scene">
            <div className="character">
              <div className="cube c-head"><div className="face front">{getIcon(user.equipped.head)}</div><div className="face right" /><div className="face top" /></div>
              <div className="cube c-body"><div className="face front">{getIcon(user.equipped.armor)}</div><div className="face right" /><div className="face top" /></div>
              ...
              <div className="cube c-leg-l"><div className="face front">{getIcon(user.equipped.shoes)}</div><div className="face right" /><div className="face top" /></div>
              <div className="cube c-leg-r"><div className="face front">{getIcon(user.equipped.shoes)}</div><div className="face right" /><div className="face top" /></div>
              {user.equipped.pet && <div className="cube c-pet"><div className="face front">{getIcon(user.equipped.pet)}</div></div>}
            </div>
          </div>
          <div className="flex justify-around items-center border-t border-slate-200 dark:border-slate-700/50 pt-6 mt-6">
            <div className="text-center"><p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">Active Streak</p><p className="font-display text-3xl text-[#f43f5e] mt-1 drop-shadow-md">{user.streak} 🔥</p></div>
            <div className="text-center"><p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">Quests Done</p><p className="font-display text-3xl text-[#8b5cf6] mt-1 drop-shadow-md">{user.completedCount} ⚔️</p></div>
          </div>
        </div>
            ...
            {BADGE_DEFS.map(b => {
              ...
              return (
                <li key={b.id} className={`flex items-center gap-4 p-3 rounded-2xl border transition-all ${earned ? 'border-[#e8c547]/50 bg-[#e8c547]/10 shadow-[0_0_15px_rgba(232,197,71,0.2)]' : 'border-slate-200 dark:border-slate-700/50 opacity-50 grayscale'}`}>
                  <span className="text-3xl filter drop-shadow-md">{earned ? "🏅" : "🔒"}</span><span className="font-bold text-base">{b.name}</span>
                </li>
              );
            })}
          </ul>
      ...
      <section className="lg:col-span-8 space-y-6">
        <div className="glass-panel rounded-[2rem] p-8 relative overflow-hidden flex flex-col md:flex-row items-center gap-6">
          <div className="flex-1 w-full">
            ...
            <div className="flex items-center gap-4">
              <div className="bg-slate-100 dark:bg-[#0f172a] px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-center"><p className="text-[10px] uppercase tracking-widest text-slate-500 font-bold">Level</p><p className="text-xl font-bold text-[#8b5cf6] dark:text-[#e8c547]">{lv.level}</p></div>
              <div className="flex-1">
                <div className="flex justify-between text-xs font-bold mb-2"><span><span className="text-lg">{user.xp}</span> XP</span><span className="text-slate-500">/ {lv.need}</span></div>
                <div className="h-4 rounded-full bg-slate-200 dark:bg-[#0f172a] overflow-hidden border border-slate-300 dark:border-slate-700 relative">
                  <div className="bar h-full bg-gradient-to-r from-[#8b5cf6] to-[#e8c547] relative" style={{ width: `${Math.min(100, (lv.into / 120) * 100)}%` }}></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleAddQuest} className="bg-slate-50 dark:bg-[#0f172a]/50 p-4 rounded-2xl border border-slate-200 dark:border-slate-700/50 mb-6 card-hover">
            <div className="grid sm:grid-cols-12 gap-3">
              ...
              <select value={qAttr} onChange={e => setQAttr(e.target.value as Attribute)} className="sm:col-span-6 md:col-span-3 px-3 py-3 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 outline-none focus:ring-2 focus:ring-[#e8c547] font-medium shadow-sm">
                <option value="strength">💪 Strength</option><option value="intellect">🧠 Intellect</option><option value="discipline">🛡️ Discipline</option><option value="social">💬 Social</option>
              </select>
              <div className="sm:col-span-6 md:col-span-2 flex items-center bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-600 px-3 shadow-sm focus-within:ring-2 focus-within:ring-[#e8c547]">
                <span className="text-xs text-slate-400 font-bold">MIN</span><input type="number" min="1" max="300" value={qMins} onChange={e => setQMins(parseInt(e.target.value)||25)} className="w-full bg-transparent border-none outline-none py-3 text-center font-bold" />
              </div>
            ...
            <div className="mt-3 text-right"><span className="text-xs font-bold text-[#8b5cf6] bg-[#8b5cf6]/10 px-3 py-1 rounded-full border border-[#8b5cf6]/20">Expected Gain: ~{calculateXP(qMins, qAttr, user.streak)} XP & {Math.max(1, Math.floor(qMins / 8))} G</span></div>
          </form>
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          <div className="glass-panel rounded-[2rem] p-6 text-center flex flex-col justify-center">
            ...
            <p className="text-base font-medium leading-relaxed my-4">The realm senses weakness in your {ATTR_LABELS[lowestAttr]}. Fortify it to achieve balance.</p>
...
function Shop({ user, patchUser, openModal }: any) {
  ...
  const cats = [{ id: "head", label: "Headgear", icon: "🪖" }, { id: "armor", label: "Outfits", icon: "👕" }, { id: "shoes", label: "Boots", icon: "🥾" }, { id: "pet", label: "Companions", icon: "🦊" }];
          ...
          return (
            <div key={i.id} className="flex flex-col items-center gap-4 p-6 rounded-[2rem] bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-700/50 card-hover text-center relative shadow-sm">
              ...
              <div className="flex-1 w-full mt-2">
                ...
                {owned ? <button onClick={() => patchUser((u:User) => { u.equipped[i.cat as keyof typeof u.equipped] = i.id; })} className={`w-full py-3 rounded-xl font-bold transition-all uppercase tracking-wide ${eq ? "bg-slate-100 dark:bg-slate-800 text-slate-400 cursor-default" : "bg-[#e8c547] text-[#0f172a] hover:scale-105 shadow-md shadow-[#e8c547]/20"}`}>{eq ? "Equipped" : "Equip"}</button>
                : <button onClick={() => { if (user.gold < i.price) return openModal({ open: true, title: "Insufficient Gold", body: `You need ${i.price - user.gold} more gold.` }); patchUser((u:User) => { u.gold -= i.price; u.inventory.push(i.id); u.equipped[i.cat as keyof typeof u.equipped] = i.id; }); }} className="w-full py-3 rounded-xl bg-[#8b5cf6] text-white font-bold hover:scale-105 transition-all shadow-[0_4px_15px_rgba(139,92,246,0.3)] uppercase tracking-wide">Purchase</button>}
              </div>
            </div>
          );
        })}
...
// LEADERBOARD
// ==========================================
function Leaderboard({ localDB }: { localDB: Record<string, User> }) {
  const [players, setPlayers] = useState<{name: string, level: number, xp: number}[]>([]);
  ...
  return (
    <div className="glass-panel rounded-[2rem] p-8 max-w-4xl mx-auto">
      <div className="text-center mb-10"><h2 className="font-display text-4xl text-[#8b5cf6] mb-3 drop-shadow-md">Hall of Heroes</h2><p className="text-base text-slate-500 font-medium">The most dedicated adventurers in the realm.</p></div>
      {players.length === 0 ? <p className="text-center text-slate-400">No heroes have registered their legends yet.</p> : (
          ...
          {players.map((r, i) => {
            ...
            return (
              <div key={i} className={`flex items-center gap-6 p-5 rounded-xl border ${style} shadow-sm`}>
                ...
                <div className="flex-1"><p className={`font-bold text-xl ${i === 0 ? 'text-[#e8c547]' : 'text-slate-800 dark:text-white'}`}>{r.name}</p><p className="text-xs font-bold uppercase tracking-widest opacity-70 mt-1">Lv {r.level} • {r.xp} XP</p></div>
              </div>
            );
          })}
        </div>
